...
if [[ " ${REACT_SITES[*]} " =~ " $SITE_NAME " && -f "$SOURCE_DIR/dist/index.html" ]]; then
    ...
    if (( DIST_MTIME > 0 && HEAD_MTIME > 0 && DIST_MTIME < HEAD_MTIME )); then
        log_warn "Gate 1g2: dist/index.html mtime ($DIST_MTIME) < HEAD commit mtime ($HEAD_MTIME) — stale dist detected. Rebuilding..."
        (cd "$SOURCE_DIR" && npm run build) || die "dist freshness auto-rebuild failed"
    fi
    log_info "  ✅ Gate 1g2 (dist freshness verified)"
fi
...
log_info "Gate 1f₂: Running Soft-200 regression check..."
...
log_info "  ✅ Gate 1f₂ passed (garbage path returned HTTP $GARBAGE_CODE, not 200)"
